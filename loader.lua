...
local function setupCharacter(character)
	...
	table.insert(characterConnections,
		rootPart:GetPropertyChangedSignal("AssemblyLinearVelocity"):Connect(clampVelocity))
	table.insert(characterConnections,
		rootPart:GetPropertyChangedSignal("AssemblyAngularVelocity"):Connect(clampVelocity))
...
local function notify(titleText, bodyText, duration)
	...
	task.delay(duration or 3, function()
		...
		-- FIX: self-disconnecting handler prevents connection accumulation on
		-- repeated notifications across a long session.
		local conn
...
local function loadRemoteChunk(url)
	...
	if not httpOk then return nil, source end

	local chunk, compileError = loadstring(source)
	if not chunk then return nil, compileError or "loadstring failed" end
...
local function checkTrajectoryApi()
	...
	if not chunk then return false, loadError, nil end

	local ok, predictorOrError = pcall(chunk)
	if not ok then return false, predictorOrError, nil end
...
local function startAcidRedirect()
	...
	task.spawn(function()
		...
		if not trajectoryOk then
			...
			if acidToggleSetState then acidToggleSetState(false, true) end
...
acidStatusLabel, acidToggleTrack, acidToggleSetState = buildToggleRow(
	contentContainer, 0, "Acid-Redirect", "Disabled", AcidRedirect.Enabled,
	function(state)
		if state then startAcidRedirect() else stopAcidRedirect() end
	end
)
...
local function closePanel()
	...
	-- FIX: self-disconnecting Completed handler — previous code left one
	-- dangling RBXScriptConnection alive per close call, accumulating over time.
	local conn
...
------------------------------------------------------------------------
-- Panel drag
-- FIX: removed the dragInput middleman that was set by header.InputChanged.
-- When the cursor moves fast it leaves the header area, and InputChanged on
-- a Frame stops firing for events outside it. The old code stopped tracking
-- the mouse position mid-drag the moment the cursor left the header bounds.
--
-- New approach: header.InputBegan records dragStart + startPos at press time.
-- The single global UserInputService.InputChanged handler reads the live
-- input position directly and computes the delta, so dragging works even
-- when the cursor is far outside the header.
------------------------------------------------------------------------
...
notify("Info", "Latest Release 6/19/2026 10:37 PM, Change: Fixed Acid", 99999999999999999)
